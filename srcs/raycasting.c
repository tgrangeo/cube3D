...
/*   Updated: 2020/10/01 14:22:48 by tgrangeo         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cube3d.h"

static void		ft_raycasting2(int x, t_params *param)
{
	param->cam_x = 2 * x / (double)param->size_x - 1;
	param->raydir.x = param->dir.x + param->plane.x * param->cam_x;
	param->raydir.y = param->dir.y + param->plane.y * param->cam_x;
	...
	{
		param->step.x = 1;
		param->sidedist.x = (param->map.x + 1.0 - param->pos.x)
			* param->deltadist.x;
	}
}

static void		ft_raycasting3(t_params *param)
{
	while (param->hit == 0)
	{
...
double			ft_raycasting(int x, t_params *param)
{
	...
	{
		param->step.y = 1;
		param->sidedist.y = (param->map.y + 1.0 - param->pos.y) *
			param->deltadist.y;
	}
	ft_raycasting3(param);
	if (param->side == 0)
		param->walldist = (param->map.x - param->pos.x + (1 - param->step.x)
			/ 2) / param->raydir.x;
	else
		param->walldist = (param->map.y - param->pos.y + (1 - param->step.y)
			/ 2) / param->raydir.y;
